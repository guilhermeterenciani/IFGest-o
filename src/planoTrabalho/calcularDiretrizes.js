...
;(function (input) {
    ...
    class Dia {
        ...
        somaTrabalhos() {
            const reducer = (accumulator, currentValue) =>
                accumulator + currentValue.duracao
    ...
    function capturarCaixas() {
        ...
        for (let trI = 0; trI < trNodes.length; trI++) {
            ...
            for (let tdI = 1; tdI < tdNodes.length; tdI++) {
                let caixaNodes =
                    tdNodes[tdI].querySelectorAll(".caixa_horario p")
    ...
    function inicializarTrabalho(caixaNode) {
        let groups =
            /(?<h1>[0-9]{2}):(?<m1>[0-9]{2}) - (?<h2>[0-9]{2}):(?<m2>[0-9]{2})/gm.exec(
                caixaNode.innerText
            ).groups
        ...
        if (tempoTotal < 660) {
            log(
                "De " +
                    ...
                    " para " +
                    diasSemana[dia2] +
                    ": Intervalo inferior a 11h entre jornadas de trabalho"
            )
            return false
        }
    ...
    function totalDiarioDia(dia, maxMin) {
        ...
        if (soma != maxMin) {
            log(
                `${
                    diasSemana[dia]
                } : Carga horária diferente (${minutosToHorasString(
                    soma
                )} horas)`
            )
            return false
    ...
    function relatorioMargemAula(tipo, porcentagem, margemErro) {
        ...
        let parcelas = 4
        let cargaSugeridoM =
            Math.round(((cargaSugeridoReal / 60) % 1) * parcelas) *
            (60 / parcelas)
        ...
        if (carga < cargaSugeridoMargem1 || carga > cargaSugeridoMargem2) {
            ...
            log(
                `Margem (${margem}%) aceitavel sugerido: entre ${minutosToHorasString(
                    cargaSugeridoMargem1
                )} e ${minutosToHorasString(cargaSugeridoMargem2)} horas`
            )
            return false
        }
    ...
    function relatorioMargemAulaEscrito(tipo, porcentagem, margemErro) {
        ...
        let margem = margemErro * 100
        if (carga < cargaSugeridoMargem1 || carga > cargaSugeridoMargem2) {
            retorno.mensagem = `Aulas: ${minutosToHorasString(
                cargaAula
            )} horas <br/>Carga cadastrada: ${minutosToHorasString(
                carga
            )} horas <br/>Carga sugerido: ${minutosToHorasString(
                cargaSugerido
            ...
            retorno.mensagem = `Aulas: ${minutosToHorasString(
                cargaAula
            )} horas <br/>Carga cadastrada: 
        ${minutosToHorasString(carga)} horas <br/> 
        horas <br/>Margem (${margem}%) aceitavel: entre ${minutosToHorasString(
                cargaSugeridoMargem1
            )} e 
    ...
    function relatorioPreparacaoPermanencia() {
        ...
        if (cargaPCorreto != cargaP || cargaPECorreto != cargaPE) {
            log(`Aulas: ${minutosToHorasString(cargaAula)} horas`)
            log(
                `Preparação cadastrada: ${minutosToHorasString(
                    cargaP
                )}/${minutosToHorasString(cargaPCorreto)} horas`
            )
            log(
                `Permanencia cadastrada: ${minutosToHorasString(
                    cargaPE
                )}/${minutosToHorasString(cargaPECorreto)} horas`
            )
    ...
    function displayAll() {
        ...
        for (j = 2; j < numOfCols; j++) {
          ...
          <p style=""><strong></strong><br>${minutosToHorasString(
              soma
          )}</p></div>`
            }
        }
        ...
        x.innerHTML = relatorioMargemAulaEscrito(
            TRABALHO_P,
            TRABALHO_P_CARGA,
            MARGEM_ERRO
        ).mensagem
