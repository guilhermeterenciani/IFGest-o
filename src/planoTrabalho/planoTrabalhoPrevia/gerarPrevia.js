...
function print(x,information){
    if(debug){if(information){console.log(information,x)}else{console.log(x)}}   
}
...
const horaRelogio = [{"horaInicio":"07:00", "horaFim":"07:45"},
...
let trNodes = document.getElementsByClassName("table table-bordered table-condensed horario diario")[0].querySelectorAll('tbody>tr>td')
for (let trI=0; trI<trNodes.length-diasUteis-1;trI++){
    if(trI%diasUteis!==0){
        id = `id-node-free-${trI}`
        trNodes[trI].id = id
        print(Math.floor((trI-1)/diasUteis),"HoraRelógio");
        schedule.push({"idNodeFree":id,"diaSemana":trI%diasUteis,"tempo":horaRelogio[Math.floor((trI-1)/diasUteis)], "node":trNodes[trI],"status":"ocupado"})
    }
}
...
let result = schedule.filter((freeTime)=>freeTime.node.childNodes.length===0);//|| freeTime.node.childNodes[0].className.includes("free-time"))
//console.log(result[0])


// Add buttons on Free Time with action buttons
if(debug){console.log(result),"Printando o result"}
...
function addClickedButtonToAListOfFreeTime(listOfFreeTime){
    ...
    for (let i =0; i<listOfFreeTime.length;i++){
        if(debug){console.log(listOfFreeTime[i].idNodeFree)}
        //listOfFreeTime[i].node.id = listOfFreeTime[i].idNodeFree
        listOfFreeTime[i].status="livre"
        listOfFreeTime[i].node.innerHTML= 
        "<a class='btn btn-success'  href='javascript:void(0)'> <i class='icon-plus icon-white'></i>Adicionar PE<span class='caret'></span></a>"+
        "<a class='btn btn-success'  href='javascript:void(0)'> <i class='icon-plus icon-white'></i>Adicionar PAT<span class='caret'></span></a>"+
        "<a class='btn btn-success'  href='javascript:void(0)'> <i class='icon-plus icon-white'></i>Adicionar P<span class='caret'></span></a>"
...
function clickOnFreeTime(event){
    ...
    let idFather = event.path[1].id
    if(debug){console.log(idFather)}
...
function addPEOnFreeTime(idFather){
    let clickedFreeTime = schedule.find((freeTime)=>freeTime.idNodeFree===idFather)
    ...
    print(clickedFreeTime)
        clickedFreeTime.node.innerHTML =`<div class="hovered preparacao-7182 caixa_horario caixa_horario_p" referer="preparacao-7182">
                                            ...
                                            <a class='btn'  href='javascript:void(0)'></i>Excluir</a>
                                    `  
...
function savePATInLocalStorage(){
    print(schedule.filter((item)=>item.status==="PAT"),"Schedule that will be save")
    let patDTO = schedule
        .filter((item)=>item.status==="PAT")
        .map((item)=>{return {"id":item.idNodeFree,"diaSemana":item.diaSemana,"horaInicio":item.tempo.horaInicio,"horaFim":item.tempo.horaFim}})  
    localStorage.setItem("PATKEY",JSON.stringify(patDTO))
}

let button = document.querySelector("#HorarioDocenteEditarForm > div.btn-group > a");
